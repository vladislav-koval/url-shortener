...
**Один из багов закреплён тестом, чтобы не повторился.** На локальной базе без применённых миграций `GetByShortCode` вернул `500` вместо `404`: ошибка Postgres `relation does not exist` (код `42P01`) не совпадала ни с одним сентинелом в маппере ошибок и уходила в generic-ветку. Этот конкретный код ошибки теперь закреплён regression-тестом.
