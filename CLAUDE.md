...
Для локальной разработки оба процесса запускаются нативно (`make run` / `make run-analytics`) и подключаются к инфраструктуре из `docker-compose.yml` (Postgres/Redis/Redpanda), поднятой точечно: `docker compose up -d postgres redis redpanda port-forwarder` — **не** голым `make env-up`/`docker compose up -d`. С деплой-PR'ов (#16/#17/#18, `ecf60ed`/`b3a80da`/`03938a2`) `docker-compose.yml` вырос из чистой dev-инфры в полный прод-стек: там же теперь `caddy`, `shortener`, `analytics`, `frontend` как обычные сервисы без `profiles`, и `frontend` собирается из соседнего репозитория `../url-shortener-front` — голый `docker compose up -d` попытается поднять и их. `postgres` при этом не публикует порт напрямую (`ports:` нет), доступ с хоста — только через `port-forwarder` (профиль `tools`, но при явном указании имени сервиса профиль не мешает). **У `redis` в `docker-compose.yml` нет volume вообще** — кеш ссылок и сессии переживают рестарты только потому, что сам контейнер ни разу не пересоздавался; `docker rm`/полный `down` уничтожит всё без предупреждения, в отличие от Postgres/Redpanda с именованными volume'ами (`env-cleanup` их и спрашивает отдельно).
...
Тот же cold-start риск возвращается и после старта: у `grpc-go` дефолтный idle timeout канала — 30 минут (`dialoptions.go`, `defaultDialOptions().idleTimeout`), после простоя без RPC resolver/balancer выключаются, и следующий запрос заново платит полную цену реконнекта (реальный инцидент — `DeadlineExceeded` на первом запросе после ~5ч простоя). `grpc.WithIdleTimeout(0)` в `NewGRPCClient` отключает idle-режим целиком — не убирать эту опцию, иначе eager `conn.Connect()` защищает только от cold-start при старте процесса, а не от него же после длительного простоя. Безопасно только потому, что `shortener`↔`analytics` — прямой container-to-container трафик внутри одной docker-сети без посредника, который мог бы молча (без TCP RST/GOAWAY) рвать простаивающие соединения; если это когда-нибудь изменится (LB между сервисами и т.п.) — понадобится ещё `grpc.WithKeepaliveParams` для проактивного обнаружения протухшего соединения, само по себе отключение idle тут не поможет.

Настоящая причина того самого инцидента с `DeadlineExceeded` — не медленный резолв `localhost` как таковой (A/AAAA резолвится за <1мс, замерено `net.LookupHost` напрямую), а DNS TXT-запрос за service config (gRFC A2, `_grpc_config.<host>`), который `grpc-go` по умолчанию делает на **каждый** резолв (`GRPC_ENABLE_TXT_SERVICE_CONFIG` дефолтится в `true`, `internal/envconfig`) и не отдаёт balancer'у уже готовые адреса, пока TXT-запрос не завершится (`dns_resolver.go: lookup()` — `lookupHost` уже отработал, но `return` происходит только после `lookupTXT`). На живом прогоне с verbose-логами `grpc-go` этот TXT-запрос реально висел 8-10+ секунд (видимо, в сеть/VPN, откуда не приходит ответ) — намного дольше ретрай-бюджета (`MaxRetries×PerTimeout` ~3с), поэтому все попытки гарантированно не успевали. `grpc.WithDisableServiceConfig()` в `NewGRPCClient` убирает этот сетевой запрос целиком (замер с ним отключённым: коннект+первый RPC — 39.5мс) — никто не публикует service config в DNS для internal/loopback-таргета, retry/timeout-политика и так задаётся явно в коде через `retry.CallOption`. Не убирать эту опцию и не пытаться чинить эту проблему увеличением `GRPC_CLIENT_PER_TIMEOUT`/`GRPC_CLIENT_MAX_RETRIES` — без неё бюджет пришлось бы задирать секунд до 30 (внутренний потолок `grpc-go` `ResolvingTimeout`), и тогда каждый холодный запрос ждал бы эти секунды вместо ошибки.
