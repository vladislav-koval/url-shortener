...
Учебный/pet-проект одного разработчика, не production-нагрузка, часто разворачивается на дешёвом сервере (docker-compose с Postgres+Redis+Kafka+приложением на одной машине). Kafka-writer **не** использует `Async: true`/неограниченную очередь `kafka-go` — вместо этого свой батчинг поверх ограниченной очереди (`QueueSize`, `segmentio.Writer`). Осознанный компромисс в пользу "лучше потерять клик, чем уронить процесс OOM'ом" — на такой машине неограниченная очередь при недоступном брокере реальный риск, не гипотетический. Потери не безмолвные: `Writer.Dropped()` считает и переполнение очереди, и ошибки отправки в Kafka, фоновая горутина раз в интервал (не на каждый дроп — лог-шторм под нагрузкой) сбрасывает дельту в лог. Не считай ни ограниченную очередь, ни дропы багом — это документированное решение, см. "Ограничения".
...
### Остановка: `Run()` + `Shutdown(ctx)`, бюджет один на процесс
...
**Ни один компонент не заводит собственный `context.WithTimeout` на остановку.** Такой контекст в проекте ровно один, в `main.go`, из `SHUTDOWN_TIMEOUT`, и передаётся параметром в каждый `Shutdown` — поэтому фаза, начавшаяся позже (дренаж writer'а), получает остаток, а не свои полные 15с. Раньше было наоборот: три `context.WithTimeout(context.Background(), ...)` в трёх местах, каждый со своим отсчётом, и суммарное выключение выходило кратным числу фаз. Не возвращать `ShutdownTimeout` в конфиги фич.
...
- `consumer.ClickConsumer` — общий на все запущенные `Run()` (их `GoroutinesCount` штук) `context.WithCancel`; `Shutdown` пишет поле `shutdownCtx`, потом зовёт `cancelFetch()` — тот же порядок и то же обоснование, что у `Writer`, но синхронизацию даёт не закрытие канала, а внутренний мьютекс `context.cancelCtx` (тоже synchronized-before, просто другой примитив).
...
В `main.go` два разных контекста с разными ролями. `groupCtx` из `errgroup.WithContext(ctx)` — сигнал "пора выключаться": отменяется по SIGTERM или по первой ошибке любого `Run`, для этого `errgroup.WithContext` и нужен, руками ничего не собирается. `shutdownCtx` из `SHUTDOWN_TIMEOUT` — бюджет, идущий параметром в каждый `Shutdown`. Между сигналом на выключение и закрытием ресурсов — барьер `select` на `stopped` (все `Run` вернулись через `g.Wait()`) и на `shutdownCtx.Done()` (бюджет исчерпан); если сработал второй, `main` завершается через `return`, не закрывая `clickWriter`/`clickReader`/`redisClient`/`pgxPool` — ими всё ещё может пользоваться зависший компонент, закрывать их в этот момент значит гонку или панику.
...
- Не абстрагировать заранее — обобщение только на второй реальный потребитель. Историческая заметка: `AsyncWriter`-обёртка с пулом горутин уже была, её убирали в пользу `Async: true` у `kafka-go`, а потом вернули снова (см. выше) — потому что ограничение памяти на целевом железе оказалось важнее веса абстракции. Принцип "не абстрагируй заранее" тут не нарушен: обобщение продиктовано реальным ограничением (дешёвый сервер), а не гипотезой на будущее.
